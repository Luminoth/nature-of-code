...
fn main() -> Result<(), ProcessingErr> {
    let screen = p5::Screen::new(300, 300, false, false, true)?;

    //example(screen)
    square(screen)
}

fn square(mut screen: p5::Screen) -> Result<(), ProcessingErr> {
    let mut angle = 0.0;
...
fn example(mut screen: p5::Screen) -> Result<(), ProcessingErr> {
